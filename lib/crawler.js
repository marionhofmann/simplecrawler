...
var Crawler = function(initialURL) {
    ...
    /**
     * Controls whether to strip anchor parameters from URL's at queue
     * item construction time.
     * @type {Boolean}
     */
    this.stripHash = false;
...
Crawler.prototype.processURL = function(url, referrer) {
    ...
    // Check if hash should be ignored
    if (crawler.stripHash) {
        hash = uri(url).search("").hash();
        url2 = url.replace(hash,'');
    }
    ...
    try {
        ...
        newUrl2 = uri(url2).absoluteTo(referrer.url).normalize();
